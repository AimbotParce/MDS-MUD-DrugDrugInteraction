...
class BERT_CNN(nn.Module):
    def __init__(self, pretrained_model_name, num_labels,
                 dropout_conv=(0.5,0.2,0.2), dropout_pool=0.5):
        super().__init__()
    ...
    def forward(self, input_ids, attention_mask):
        ...
        x = F.relu(self.conv1(x)); x = self.drop1(x)
        x = F.relu(self.conv2(x)); x = self.drop2(x)
        x = F.relu(self.conv3(x)); x = self.drop3(x)
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        model.eval(); total_val, correct = 0, 0
        with torch.no_grad():
            for batch in val_loader:
                ...
                labels = batch["label"].to(device)
                logits = model(ids, mask)
                loss = criterion(logits, labels); total_val += loss.item()
                preds = torch.argmax(logits, dim=1)
                correct += (preds == labels).sum().item()
        avg_val = total_val / len(val_loader)
        acc = correct / len(val_ds)
        print(f"Epoch {epoch+1}/{args.epochs} - train_loss: {avg_train:.4f} - val_loss: {avg_val:.4f} - val_acc: {acc:.4f}", file=sys.stderr)
